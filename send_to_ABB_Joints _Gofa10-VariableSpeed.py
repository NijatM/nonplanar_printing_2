...
from npp2.tasks import RoboticFreeMovement, RoboticLinearMovement, AirAssistOn, AirAssistOff, ExtruderOn, ExtruderOff

PATH = os.path.dirname(os.path.abspath(__file__))
FILE_NAME = 'design/241121_victor/path_0.result'
...
def execute_robotic_free_movement(abb, task):
    ...
    trajectory = movement.trajectory.points
    ...
    for trajectory_point in trajectory:
        ...
        joint_values = [float(value) for value in to_degrees(trajectory_point.joint_values)]
...
def execute_air_assist_on(abb, task):
    instruction = rrc.SetDigital('ABB_Scalable_IO1_0_DO1', 0, feedback_level=1)
    result = abb.send(instruction)
    task.execution_result = [result]


def execute_air_assist_off(abb, task):
    instruction = rrc.SetDigital('ABB_Scalable_IO1_0_DO1', 1, feedback_level=1)
    result = abb.send(instruction)
    task.execution_result = [result]


def execute_extruder_on(abb, task):
    instruction = rrc.SetDigital('ABB_Scalable_IO1_0_DO2', 0, feedback_level=1)
    result = abb.send(instruction)
    task.execution_result = [result]


def execute_extruder_off(abb, task):
    instruction = rrc.SetDigital('ABB_Scalable_IO1_0_DO2', 1, feedback_level=1)
    result = abb.send(instruction)
    task.execution_result = [result]
...
def print_process():
    ...
    # --- Set tool
    abb.send_and_wait(rrc.SetTool('lilli902'))
